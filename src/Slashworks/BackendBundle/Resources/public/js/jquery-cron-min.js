(function (e) {
    ...
    var l2 = ["Januar", "Februar", "März", "April", "Mai", "Juni", "Juli", "August", "September", "Oktober", "November", "Dezember"];
    for (var u = 0; u < l.length; u++) {
        h += "<option value='" + (u + 1) + "'>" + l2[u] + "</option>\n"
    }
    var s = "";
    var g = ["Sunday", "Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday"];
    var g2 = ["Sonntag", "Montag", "Dienstag", "Mittwoch", "Donnerstag", "Freitag", "Samstag"];
    for (var u = 0; u < g.length; u++) {
        s += "<option value='" + u + "'>" + g2[u] + "</option>\n"
    }
    var r = "";
    var b = ["minute", "hour", "day", "week", "month", "year"];
    var b2 = ["Minute", "Stunde", "Tag", "Woche", "Monat", "Jahr"];
    for (var u = 0; u < b.length; u++) {
        r += "<option value='" + b[u] + "'>" + b2[u] + "</option>\n"
    }
    var p = {
        minute: [],
        hour: ["mins"],
    ...
    var x = {
        init: function (i) {
            ...
            C.period = e("<span class='cron-period'>Jede(n) <select name='cron-period'>" + A + r + "</select> </span>").appendTo(this).data("root", this);
            var E = C.period.find("select");
            E.bind("change.cron", m.periodChanged).data("root", this);
            if (B.useGentleSelect) {
                E.gentleSelect(j)
            }
            C.dom = e("<span class='cron-block cron-block-dom'> am <select name='cron-dom'>" + v + "</select> </span>").appendTo(this).data("root", this);
            E = C.dom.find("select").data("root", this);
            if (B.useGentleSelect) {
                E.gentleSelect(B.domOpts)
            }
            C.month = e("<span class='cron-block cron-block-month'> am <select name='cron-month'>" + h + "</select> </span>").appendTo(this).data("root", this);
            E = C.month.find("select").data("root", this);
            if (B.useGentleSelect) {
                E.gentleSelect(B.monthOpts)
            }
            C.mins = e("<span class='cron-block cron-block-mins'> um <select name='cron-mins'>" + y + "</select> Minuten nach gewählter Stunde </span>").appendTo(this).data("root", this);
            E = C.mins.find("select").data("root", this);
            if (B.useGentleSelect) {
                E.gentleSelect(B.minuteOpts)
            }
            C.dow = e("<span class='cron-block cron-block-dow'> am <select name='cron-dow'>" + s + "</select> </span>").appendTo(this).data("root", this);
            E = C.dow.find("select").data("root", this);
            if (B.useGentleSelect) {
                E.gentleSelect(B.dowOpts)
            }
            C.time = e("<span class='cron-block cron-block-time'> um <select name='cron-time-hour' class='cron-time-hour'>" + d + "</select>:<select name='cron-time-min' class='cron-time-min'>" + y + " </span>").appendTo(this).data("root", this);
